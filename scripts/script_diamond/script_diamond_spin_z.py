...
for i in range(indeces_labels.shape[0]):
    ...
    sigma_x_A = np.dot(mini_spinor_A.conj(), np.dot(sigma_z, mini_spinor_A))
    sigma_x_B = np.dot(mini_spinor_B.conj(), np.dot(sigma_z, mini_spinor_B))
    print(sigma_x_A)
...
ax.set_yticks([])
import os
plt.savefig(os.path.join("plots/spin", "sigma_z_component_n1_"+str(n1)+"_n2_"+str(n2)+".png"))
...
for i in range(indeces_labels.shape[0]):
    ...
    if Labels[tuple(indeces_labels[i])] == 1:
        ax.plot(coordA[0], coordA[1], color = cmap(np.real_if_close(sigma_x_A)), marker = "o", ls = "")
        ax.plot(coordB[0], coordB[1], color = cmap(np.real_if_close(sigma_x_B)), marker = "o", ls = "")
...
ax.set_yticks([])
import os
plt.savefig(os.path.join("plots", "sigma_x_component_n1_"+str(n1)+"_n2_"+str(n2)+".png"))